 ...
 * [perf] `encode`, `encode_strictly`, `decode`, and `decode_exactly` now take a fast path for canonical inputs: in-range exact `float` coordinates with an in-range `int` precision go straight to the C encoder, and non-empty in-bounds strings are handed to the C codec verbatim with a case-normalized retry on its rejection instead of an unconditional `.lower()`. Typical encodes are ~20-25% faster and decodes ~8-10% faster. Validation outcomes, error messages, case-insensitive handling, bool rejection, NaN/infinite-coordinate rejection, precision bounds, and all pinned values are unchanged. Case-variant (uppercase/mixed-case) decode inputs pay one extra codec validation attempt before the case-normalized retry.
